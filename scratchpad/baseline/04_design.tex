...
% Rejected structural alternatives and supplementary calibrations appear in Appendices~\ref{app:rationale} and~\ref{app:supplementary}.
...
Patterns 1, 3, and 4 represent out-of-sample or off-vintage benchmarks used as secondary plausibility checks (Section~\ref{sec:lim-validation}).
...
Comparing simulated output to Pattern~2 requires two structural caveats.
First, \ac{CCMR} administrative data counts individual accounts, whereas the simulation operates at the household level; calibration targets the shape and scale of the arrears distribution rather than point estimates.
Second, non-indebted households cannot enter arrears by definition. Arrears proportions are therefore calculated over the credit-active subpopulation (the 44\% of households holding traditional debt), while the population-wide default rate is tracked separately to address the primary research questions.

\begin{table}[H]
\centering
\caption{Empirical patterns registered before any result was produced. Pattern 2 is the
calibration target; Patterns 1, 3 and 4 are foreign or off-vintage and are read as plausibility
checks (Section~\ref{sec:lim-validation}).}

% Appendix~\ref{app:caveats} records the input-quality
% caveat on Pattern 4.}
\label{tab:patterns}
...
\begin{table}[H]
\centering
\caption{2017-Q1 \ac{CCMR} arrears profile, unsecured credit and credit facilities combined, on an
account basis. Tables 21 and 23 \cite{ncr_ccmr_2017}. Aggregate bands are cumulative.}
\label{tab:ccmr}
\begin{tabular}{lr}
\toprule
...
The dashed connections highlight key regulatory reporting gaps: credit platforms lack visibility into competitor extensions, and \ac{BNPL} liabilities are not reported to the central bureau.
...
\draw[gap] (plats.north) -- (bureau.south) node[lbl, midway, right] {no reporting duty\\ (Lever 1 creates it)};
\draw[sees] (group) -- (hh) node[lbl, midway, left] {lagged adoption\\ share $s_g$};
\end{tikzpicture}
\caption{Information structure of the model. Solid arrows indicate observed information flows. Dashed connections mark information asymmetries in the South African baseline: platforms operate without inter-firm visibility, and origination data does not flow to the credit bureau unless explicit reporting requirements (Submodel~15, Lever 1) are enabled. Consequently, the traditional lender evaluates Regulation 23A affordability constraints using truncated bureau files, while households observe only their individual accounts and lagged peer adoption rates.}
...
Individual household tick operations proceed as follows:

\begin{enumerate}
    \item Wage-earning members undergo stochastic employment shocks according to baseline separation and hazard rates; household labor income updates accordingly.
    \item Non-discretionary expenses (food, housing rent) are settled from cash reserves.
    \item Debt interest accrues and scheduled obligations fall due (contractual installments for scheduled-payers or interest plus minimum principal for minimum-payers, alongside active \ac{BNPL} payments and accrued arrears).
    \item Liquidity shortfalls trigger credit applications, routing first to \ac{BNPL} channels (if eligible) and subsequently to traditional lenders. Unmet obligations following credit decisions determine financial distress.
    \item Debt payments execute, prioritizing automated \ac{BNPL} card debits. Payment friction may delay affordable traditional debt payments, causing unserviced balances to transition into arrears.
    \item Remaining liquid balances fund discretionary consumption down to the subsistence floor.
    \item Wants-driven \ac{BNPL} purchases execute subject to eligibility, cooling-off rules, and peer adoption metrics. Household balances, arrears duration, distress states, and default flags update.
\end{enumerate}
...
Table~\ref{tab:design-concepts} outlines the eleven structural \ac{ODD} design concepts.
...
\caption{Submodel specification. Every behavioural rule is grounded in literature or flagged as
an assumption; every free parameter is sourced, derived, or swept.}
\label{tab:submodels} \\
\toprule
...
\addlinespace
10. Information asymmetry & The bureau records traditional debt and defaults only. Lenders cannot observe \ac{BNPL} or informal debt. & \cite{nortonrose_bnpl_sa, transunion_cps_sa_2025} & \texttt{bureau\_visibility} boolean (default off, the status quo); Lever 1 in Submodel 15. \\
\addlinespace
...
\addlinespace
15. Intervention levers & Bureau visibility, mandatory Regulation 23A on \ac{BNPL}, a 14-day cooling-off period, and hypothetical concurrent-facility caps. & \cite{fca_ps26_1, woolard2021} & Binary triggers. Judged on systemic debt mitigated versus merely deferred. \\
\addlinespace
...
Bureau reporting is toggled via a boolean parameter (\texttt{bureau\_visibility}), set to false in baseline runs to reflect South African financial regulations.
Enabling bureau visibility incorporates active \ac{BNPL} obligations into formal affordability checks, isolating the systemic impact of unobserved credit.
...
We test four regulatory policy interventions to address the third research question:
\begin{enumerate}
\item \textbf{Bureau visibility:} Mandates credit bureau reporting for all \ac{BNPL} liabilities, eliminating the reporting asymmetry in Submodel~10.
\item \textbf{Mandatory affordability screening:} Enforces statutory Regulation 23A affordability checks on \ac{BNPL} originations, aligning with UK regulatory frameworks \cite{fca_ps26_1, woolard2021}.
\item \textbf{Cooling-off windows:} Restricts discretionary wants-based originations for one tick (14 days) following an order, while preserving shortfall-driven borrowing access.
\item \textbf{Facility limits:} Enforces strict caps on total concurrent platforms per household (classified as a hypothetical scenario, as no jurisdiction currently mandates multi-platform caps).
\end{enumerate}
Policy effectiveness is evaluated via cumulative \ac{BNPL} volume metrics, distinguishing temporary borrowing deferrals from permanent credit reduction.
...
% The parameter register in Appendix~\ref{app:specification} is generated from the code's own parameter declarations, so the table and the implementation cannot drift apart.
...
Model calibration targets two empirical parameters: the wage shock onset probability (calibrated to match the \ac{CCMR} 90+ day default share) and the payment friction rate (calibrated to the 1--30 day arrears share).
All remaining parameters are derived from survey data or systematically evaluated across pre-registered parameter sweeps.
Experiments are executed across multi-seed Monte Carlo runs; sensitivity analysis, noise tolerances, and full parameter specifications are detailed in Section~\ref{sec:results-robustness}.

% Also include specification appendix
