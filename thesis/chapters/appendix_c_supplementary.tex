...
The baseline is calibrated, not validated & Two parameters are fitted to two \ac{CCMR} bands; agreement on those bands is not evidence, and the fitted shock rate is about 1.4 times the upper bound of the labour-force separation rate because one channel does the work of several & Section~\ref{sec:lim-calibration}; bounds every baseline agreement \\
\addlinespace
...
scenarios are (Tables~\ref{tab:cooloff} and~\ref{tab:cap}; Figure~\ref{fig:scenarios-appendix}
places them beside the body scenarios). The cap is a count of open agreements on any platform,
not of platforms: a cap of one is a one-agreement-at-a-time rule, and it is stricter than the
single-platform arm of Table~\ref{tab:stacking}, on which a household may hold several
agreements at once.

\input{chapters/generated/tab_cooloff}

\input{chapters/generated/tab_cap}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/fig_scenarios_appendix.pdf}
\caption{Every lever against the benchmark, with and without peer influence}
\label{fig:scenarios-appendix}
\tabnote{The population default rate at the final tick under every implemented lever, at $\beta = 0$ (left) and $\beta = 1$ (right): the two body scenarios of Section~\ref{sec:scenarios}, the cooling-off window of one to four ticks and the concurrent-facility cap of one to three agreements. Points are means over 20 replicates with the replicate standard deviation; the dashed line is the benchmark mean at the same $\beta$. The cap is hypothetical: no jurisdiction imposes one. Values and differences with standard errors are in Tables~\ref{tab:scenarios}, \ref{tab:cooloff} and~\ref{tab:cap}.}
\end{figure}
...
summarises in one sentence; Table~\ref{tab:robustness-appendix} the sensitivity arms whose range
does not exceed replicate noise, including the activation-order and population-size checks; and
...
\input{chapters/generated/tab_sobol}

\input{chapters/generated/tab_robustness_appendix}

\input{chapters/generated/tab_stacking_full}
