...
\addlinespace
Borrowing rules lack direct estimates & The shortfall amount and its BNPL cap lack direct empirical estimates; their effects require sensitivity analysis & Section~\ref{sec:amount}; bounds the default level and the size, not the sign, of the \ac{BNPL} effect (Table~\ref{tab:effect}) \\
\addlinespace
...
\cite{fca_ps26_1}; and a cap on the number of concurrent agreements a household may hold, which
...
\tabnote{The population default rate at the final tick under every implemented lever, at $\beta = 0$ (left) and $\beta = 1$ (right): the two body scenarios of Section~\ref{sec:scenarios}, the cooling-off window of one to four ticks and the concurrent-agreement cap of one to three agreements. Points are means over 20 replicates with 95\% confidence intervals of the mean; the dashed line is the benchmark mean at the same $\beta$. The cap is hypothetical: no jurisdiction imposes one. Values and differences with standard errors are in Tables~\ref{tab:scenarios}, \ref{tab:cooloff} and~\ref{tab:cap}.}
\end{figure}
