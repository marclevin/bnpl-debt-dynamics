...
\addlinespace
One rule has no anchor & The shortfall borrowing amount is uncited and is the largest remaining sensitivity & Section~\ref{sec:amount}; bounds the default level \\
\addlinespace
The model cannot produce a cascade & No channel carries one household's default to another's income, so defaults are correlated individual events & Table~\ref{tab:design-concepts}; Section~\ref{sec:no-cascade}; bounds the negative result of RQ2 \\
\addlinespace
...
so opening servicing is an upper bound. Repayment horizons are sourced, two measured in
