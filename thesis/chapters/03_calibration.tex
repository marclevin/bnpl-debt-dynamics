...
Table~\ref{tab:bnpl-on-checks} collects the checks on the \ac{BNPL}-on model, none of them fitted.
At the end of the horizon 45.0\% of \ac{BNPL} holders hold two or more facilities; the over-the-horizon share, 37.0\% of ever-adopters, is the closer analogue of the \ac{CFPB}'s 32\%.
The registered mean-purchase check passes at the edge of its band: R645.60 against R992, 34.93\% low against a 35\% tolerance, inside by R0.74, with eight of twenty replicates inside.
Annual \ac{BNPL} volume per eligible household, R861, and per adopting household, R1{,}020, both fall below the provider band's R1{,}333 lower bound.
Pattern~1 is mixed, Pattern~3 holds on the registered statistic, and Pattern~4 is of the right order (25.9\% against 36\%).
RQ1 is answered in part: the population reproduces the fitted bands, three of four external data-layer checks and the stacking and purchase-size orders of magnitude, and fails the intermediate arrears bands, the servicing-share check, the volume band and the interest half of Pattern~1.
