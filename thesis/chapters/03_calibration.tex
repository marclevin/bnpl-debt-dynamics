...
and rent values in fields recorded for holders are set to zero;
...
The net effect on default is unsigned.
...
remains unidentified. The banking ceiling, the \ac{CFPB} stacking shares and
TransUnion's finding that 57\% of surveyed consumers hold a \ac{BNPL} product
\cite{transunion_cps_sa_2025}, against 69.9\% of households ever holding a
balance at $\beta=0$, offer only indirect comparisons. To address RQ2, we vary platforms from one to six and
...
short: R861 per eligible household against the R1{,}333 lower bound, and
R1{,}020 per adopter against the R4{,}261 bound for active customers.
Pattern~1 fails on one of its two measures: enabling \ac{BNPL} raises
traditional arrears but lowers the interest charged on traditional debt,
which the condition in Table~\ref{tab:patterns} counts as falsifying.
Pattern~3 holds for the specified aggregate statistic, and Pattern~4 is of
the same order, 25.9\% against 36\%. That share is a post-burn-in mean. It
lies below the 44.3\% at initialisation because the model saves any cash left
