...
Section~\ref{sec:model} stated the rules; this section states where their numbers come from.
Every input belongs to one of five classes, set out in Table~\ref{tab:param-taxonomy}: observed in South African data, constructed from it, imported from foreign literature, fitted, or swept because nothing anchors it.
Only two parameters are fitted.
The subsections follow that order, from the simulation protocol and the household population to estimation and the validation record; Appendix~\ref{app:specification} lists every parameter with its value, provenance and sweep range.
...
Each run simulates 5{,}000 households for 52 ticks (24 calendar months) and discards the first 12 ticks as initialisation burn-in.
...
Purchase size is set by no provider: $\kappa = 0.139$ is the share of discretionary spending going to the categories \ac{BNPL} finances, derived from IES 2022/23 microdata \cite{statssa_ies_2023}, and because only a ratio is imported the later vintage cannot contaminate a 2017-Rand model.
...
The fit gives a shock probability of 4.0\% per tick and a friction rate of 9\% per tick, and both are then held fixed in every run, so nothing is re-tuned once \ac{BNPL} is injected.
...
The fitted value is also implausibly high, more than three times the upper bound of the separation rate the labour force survey records (0.54--1.17\% per tick), because the model has exactly one thing that can go wrong where real households also fall behind through illness, unexpected bills, rate rises and family change: one channel does the work of several.
...
The strongest single check is the part of the arrears profile that was left free (Table~\ref{tab:arrears-fit}).
The 60+ day share, fitted by nothing, is 15.0\% against the \ac{CCMR}'s 16.5\%.
The two intermediate bands fall short, at about 1\% each against 3.6\% and 2.3\%: arrears in the model are bimodal, because an unemployment spell at the sourced re-employment hazard lasts about as long as the simulation horizon, so a household either cures within a month or slides through to the tail.

\begin{table}[H]
\centering
\caption{Baseline arrears profile against the \ac{CCMR}}
\label{tab:arrears-fit}
\tabnote{The model's baseline arrears distribution over credit-active households, \ac{BNPL} disabled, against the \ac{CCMR} shares of Table~\ref{tab:ccmr}, in per cent. Means over twenty replicates (seeds 1000--1019) at the fitted shock probability of 0.040 and friction rate of 0.09; the replicate standard deviation of the 90+ day share is 0.39 percentage points. ``Fitted'' marks the two bands the two parameters were estimated on, where agreement is not evidence; every other row is out of sample. The 60+ row is cumulative.}
\begin{tabular}{lrrl}
\toprule
\textbf{Age band} & \textbf{\ac{CCMR}} & \textbf{Model} & \textbf{Status} \\
\midrule
Current       & 71.63 & 75.61 & unfitted \\
1--30 days    &  8.24 &  8.32 & fitted \\
31--60 days   &  3.59 &  1.06 & unfitted \\
61--90 days   &  2.32 &  1.05 & unfitted \\
90+ days      & 14.21 & 13.96 & fitted \\
\midrule
\emph{60+ days} & \emph{16.54} & \emph{15.01} & \emph{unfitted} \\
\bottomrule
\end{tabular}
\end{table}
...
%   - realised mean purchase against the R992 issuer-disclosed order value. NOTE: this
%     check currently MISSES (R629, -37%; DEFECTS.md B32) and the comparison population
%     is an open decision. Report whatever is decided as decided; do not tune a tolerance.
%   - Pattern 1 direction; Pattern 3 DTI peak; Pattern 4 zero savings (32.1% at the
%     re-fitted baseline against TransUnion's 36%)
...
All fourteen pass, and none is evidence about the population beyond the two surveys it was built from; a fifteenth bounds how often the affordability guard binds.
...
Two of the three pass; the servicing share of monthly outlay among households holding a modelled product is 12.8\%, above the 6.1--9.8\% survey range, the excess being in the direction expected from ceiling rates.
