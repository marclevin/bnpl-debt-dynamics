...
\Function{BnplDraw}{$i$, amount} \Comment{returns the gross amount financed}
    \If{$\text{amount} \leq 0$, or $\bar n$ is set and $i$ already holds $\bar n$ agreements} \Return 0 \EndIf
...
\Function{Request}{$p$, $i$, amount} \Comment{the platform's light screen, blind to other platforms}
    ...
    \State Open an agreement at $p$: principal amount, instalment amount$/N$, $N - 1$ instalments remaining
    \State \Return amount
\EndFunction
\end{algorithmic}
\end{algorithm}
