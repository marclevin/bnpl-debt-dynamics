...
for the amount Submodel~4 specifies, takes \ac{BNPL} first where it can, up to $\kappa$ times
its monthly discretionary budget $E^d_i$ (the cap $\mathbb{1}^{\text{cap}}$, on in every arm but
the robustness one), and applies to the traditional lender for the remainder. The lender's test is the residual-income calculation on
...
\ac{BNPL} instalments are added to the visible obligations. A granted loan is booked: it joins the
...
\Function{SeekCredit}{$i$, shortfall}
    ...
    \EndIf
    \If{$\text{amount} - \text{financed} > 0$}
        \State $\text{granted} \gets \Call{LenderApply}{i,\ \text{amount} - \text{financed}}$
