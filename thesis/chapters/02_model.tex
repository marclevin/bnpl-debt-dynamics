...
This chapter explains who acts in the model, what each institution can observe,
...
additional credit channel within this model; it is not a forecast for a later
calendar year.
...
\ac{BNPL}. These are explicit modelling assumptions representing the reporting
gaps described in Section~\ref{sec:background}.
...
The boolean parameter \texttt{bnpl\_bureau\_visible} controls whether active
\ac{BNPL} obligations enter this calculation. It is false in the benchmark.
Switching it on adds those obligations to the traditional lender's affordability
assessment, representing one possible channel of the reporting arrangements
discussed in Section~\ref{sec:background}.

This switch does not create a credit score or change lending prices. It can
change approval through the affordability calculation, but cannot represent
score-based refusals. This distinction is in place because FINASA reports preliminary
bureau analysis suggesting potential score reductions for roughly a quarter of
credit-active consumers \cite{finasa_bnpl_2026}. That scoring channel remains
outside the comparison in Section~\ref{sec:scenarios} and is discussed as an
...
converted to tick amounts using $12/26$, where $26$ represents the number of ticks in a year and $12$ represents the number of months. Households act in a newly randomised
...
The order determines how a shortfall develops. Paying committed expenditure
first allows essential living costs to leave insufficient cash for debt service,
consistent with a cash-flow approach to distress \cite{madeira2018chile}.
Allowing credit applications before repayment gives borrowing a role in covering
the gap \cite{dorazio2017micro}. Distress is assessed before payment friction,
...
Section~\ref{sec:population}, with zero opening \ac{BNPL} balances.
...
The baseline request amount matches the shortfall. Two alternatives add either 25\% or one
...
using the same budget share that sizes want-driven purchases. This is a modelling
approximation, not an empirically established maximum. The sensitivity analysis
therefore repeats each borrowing-amount rule with the cap removed.
...
motivates this channel \cite{ackert2025bnpl, cardaci2018inequality}, but does
not identify its local strength. Both parameters are swept, with $\beta=0$
...
$\mathcal{N}(\mu_\theta, \sigma_\theta)$ and clipped to $[0,1]$. Negative
draws become zero, allowing some households to respond immediately.
...
Each platform limits its own exposure to a household without observing balances
elsewhere. Repeated borrowing can therefore create concurrent obligations across
platforms, with no aggregate exposure cap in the benchmark. The model does not
...
agreements. The stacking measures are compared with the \ac{CFPB} evidence
introduced in Section~\ref{sec:background}, with differences in denominators
and observation periods discussed in the validation chapter.

Two switches define the regulatory comparisons in Section~\ref{sec:scenarios}.
The bureau switch, $\mathbb{1}^{\text{bur}}$, adds \ac{BNPL} obligations to
the traditional lender's affordability calculation. The screening switch,
$\mathbb{1}^{\text{aff}}$, applies the affordability gate to \ac{BNPL}
originations. Algorithms~\ref{alg:gate} and~\ref{alg:bnpl} specify how the
switches enter the decisions.
...
generated from the code's declarations and requires regeneration if they change.
...
$\gamma=0$), and restrict access to one platform.
