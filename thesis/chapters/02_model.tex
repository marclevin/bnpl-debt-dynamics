...
We model the South African household credit market as an \ac{ABM} in which survey-derived households earn, spend, borrow, repay and default, a single traditional lender applies the statutory affordability test, and \ac{BNPL} platforms lend outside it.
This section describes the mechanism in the order a reader needs it: the agents and institutions, what each of them can observe, the sequence of events within a tick, and the rules governing credit and adoption.
Where each number comes from is the subject of Section~\ref{sec:calibration}.
The specification follows the \ac{ODD} protocol \cite{grimm2006odd, grimm2020odd}: Table~\ref{tab:submodels} is the definitive statement of the rules, the protocol's design concepts and the mechanisms excluded by design are in Table~\ref{tab:design-concepts} (Appendix~\ref{app:specification}), and the pseudocode of Appendices~\ref{app:pseudo-init} and~\ref{app:pseudo-main} is the normative statement of every rule below; rejected structural alternatives are in Appendix~\ref{app:rationale}.

\label{sec:purpose}
The simulation employs a counterfactual injection design.
Each household initialises with a zero \ac{BNPL} balance, the macroeconomic environment is fixed at 2017 baseline conditions, and the reference model is calibrated with \ac{BNPL} disabled.
...
The bureau-visibility contrast of Section~\ref{sec:scenarios} is therefore a statement about the affordability channel only; its near-null result is not evidence that the requirement will have no effect, and a scoring rule is the extension the 2026 change most directly motivates (Section~\ref{sec:future-work}).
...
A household is distressed in a tick when income, savings and any credit raised cannot cover committed expenditure plus scheduled service.
...
It is the model's first uncited rule, so all three specifications in Table~\ref{tab:submodels} are run and Section~\ref{sec:results-robustness} reports what the choice moves.
...
A non-zero baseline propensity ($q_{\text{base}} > 0$) ensures non-zero adoption at initialisation when aggregate balances are zero.
...
Degenerate parameter sweeps confirm baseline stability: disabling \ac{BNPL} recovers baseline state conditions, setting $\beta = 0$ and $\gamma = 0$ reproduces single-agent behaviour, limiting platforms to one eliminates cross-platform stacking, and adjusting payment friction alters arrears distributions without shifting core default rates.
