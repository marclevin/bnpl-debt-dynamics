...
The limitations of Chapter~\ref{ch:limitations} suggest five extensions, ordered by the ratio of
what they would add to what they would cost.

\textbf{Bounding the adoption parameters.} The most valuable improvement available to this model is
to constrain $q_{\text{base}}$ using the \ac{CFPB} stacking shares as a calibration target and not
only as an after-the-fact check (Section~\ref{sec:lim-uncalibrated}). This would convert a free
parameter into a partly calibrated one and strengthen the first research question, at the cost of
one additional calibration loop, with no new data and no structural change.
...
(Section~\ref{sec:lim-structural}). Three candidates would change that: a traditional lender with a
...
\cite{cardaci2018inequality}, in which peer influence acts on consumption. Any of the three would
let a successor model ask the systemic question this one does not.
...
Section~\ref{sec:lim-transfer} cannot be solved from within a modelling exercise. Eliciting present
bias, minimum-payment anchoring, or \ac{BNPL} social-approval effects in a South African sample
would replace the model's least defensible imported parameters with domestic estimates.
